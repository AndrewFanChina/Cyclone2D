 ...
 */
public class C2D_XmlParser extends C2D_AbstractXmlParser
{
	...
	C2D_StartTag current;
	/** The next event. May be null at startup. */
	protected C2D_ParseEvent next;
	...
	public String readName() throws IOException
	{
		...
		{
			c = peekChar();
			if (c < 128 && c != '_' && c != '-' && c != ':' && c != '.' && (c < '0' || c > '9') && (c < 'a' || c > 'z')
					&& (c < 'A' || c > 'Z'))
				break;
	...
	class DefaultParserException extends C2D_ParseException
	{
		DefaultParserException(String msg, Exception chained)
		{
			super(msg, chained, C2D_XmlParser.this.line, C2D_XmlParser.this.column);
		}
	}

	public C2D_XmlParser(Reader reader) throws IOException
	{
		this(reader, 8192);//Runtime.getRuntime().freeMemory() >= 1048576 ? 8192 : 1
	}

	public C2D_XmlParser(Reader reader, int bufSize) throws IOException
	{
	...
	C2D_ParseEvent parseComment() throws IOException
	{
		StringBuffer buf = new StringBuffer();
		if (readChar() != '-')
		...
		while (cnt-- > 2)
			buf.append('-');
		return new C2D_ParseEvent(C2D_Xml.COMMENT, buf.toString());
	}

	/* precondition: &lt! consumed */
	C2D_ParseEvent parseDoctype() throws IOException
	{
		StringBuffer buf = new StringBuffer();
		int nesting = 1;
		while (true)
		{
			...
			case '>':
				if ((--nesting) == 0)
					return new C2D_ParseEvent(C2D_Xml.DOCTYPE, buf.toString());
				break;
			}
			buf.append((char) i);
		}
	...
	C2D_ParseEvent parseCData() throws IOException
	{
		...
		return new C2D_ParseEvent(C2D_Xml.TEXT, buf.toString());
	}

	/* precondition: &lt;/ consumed */
	C2D_ParseEvent parseEndTag() throws IOException
	{
		skipWhitespace();
		...
		while (true)
		{
			...
			{
				if (relaxed)
					return new C2D_ParseEvent(C2D_Xml.END_DOCUMENT, null);
		...
		C2D_XmlTag result = new C2D_XmlTag(C2D_Xml.END_TAG, current, current.namespace, current.name);
		current = current.parent;
		return result;
	}

	/** precondition: <? consumed */
	C2D_ParseEvent parsePI() throws IOException
	{
		StringBuffer buf = new StringBuffer();
		readTo('?', buf);
		readChar(); // ?
		...
		readChar(); // consume >
		return new C2D_ParseEvent(C2D_Xml.PROCESSING_INSTRUCTION, buf.toString());
	}

	C2D_StartTag parseStartTag() throws IOException
	{
		...
		while (true)
		{
			...
			attributes.addElement(new C2D_XmlAttribute(null, attrName, buf.toString()));
			if (delimiter != ' ')
				readChar(); // skip endquote
		}
		try
		{
			current = new C2D_StartTag(current, C2D_Xml.NO_NAMESPACE, qname, attributes, immediateClose,
					processNamespaces);
		}
		catch (Exception e)
		{
	...
	/** precondition: &lt; consumed */
	C2D_ParseEvent parseSpecial() throws IOException
	{
		switch (peekChar())
		{
		case -1:
	...
	public C2D_ParseEvent read() throws IOException
	{
		if (next == null)
			peek();
		C2D_ParseEvent result = next;
		next = null;
		return result;
	}

	public C2D_ParseEvent peek() throws IOException
	{
		if (next == null)
		{
			if (immediateClose)
			{
				next = new C2D_XmlTag(C2D_Xml.END_TAG, current, current.namespace, current.name);
			...
			else
			{
				...
				case -1:
					...
					next = new C2D_ParseEvent(C2D_Xml.END_DOCUMENT, null);
					break;
				default:
				{
					StringBuffer buf = new StringBuffer();
					int type = readText(buf, '<');
					next = new C2D_ParseEvent(type, buf.toString());
				}
				}
			}
		}
