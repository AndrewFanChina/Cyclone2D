...
public class JPEGDecoder
{
	...
	private int x = 0, y = 0, num = 0; // the begin point of MCU
	private int IDCT_Source[] = new int[64];//64
	private final int IDCT_P[] =
	{
			0, 5, 40, 16, 45, 2, 7, 42, 21, 56, 8, 61, 18, 47, 1, 4, 41, 23, 58, 13, 32, 24, 37, 10, 63, 17, 44, 3, 6,
			43, 20, 57, 15, 34, 29, 48, 53, 26, 39, 9, 60, 19, 46, 22, 59, 12, 33, 31, 50, 55, 25, 36, 11, 62, 14, 35,
	...
	protected final void error(String message) throws Exception
	{
		throw new Exception(message);
	}

	protected final int get8(InputStream in) throws Exception
	{
		try
		{
			return in.read();
	...
	//get  16-bit data 
	protected final int get16(InputStream in) throws Exception
	{
		int temp;
		try
		{
	...
	//in-place operation
	protected void EnhanceQuantizationTable(int qtab[])
	{
		int i;
		for (i = 0; i < 8; i++)
		{
	...
	protected int  MSB = 0x80000000;
	// max size = MAX_HUFFMAN_SUBTREE * 256
	protected int  MAX_HUFFMAN_SUBTREE = 50;
	protected int HuffTab[][][] = new int[4][2][MAX_HUFFMAN_SUBTREE * 256];

	protected void Build_HuffTab(int tab[], int L[], int V[][]) throws Exception
	{
	...
	private int HuffmanValue(int table[], int temp[], int index[], InputStream in) throws Exception
	{
		...
		index[0] += 8 - (code >> 8);
		if (index[0] < 0)
			error("index=" + index[0] + " temp=" + temp[0] + " code=" + code + " in HuffmanValue()");
	...
	protected void level_shift(int du[], int P) throws Exception
	{
		int i;
		if (P == 8)
		{
	 ...
	 * @param temp
	 * @param index
	 * @return return 0       if correctly decoded, 0xFF??  if it sees a MARKER
	 * @throws Exception
	 */
	private int decode_MCU(InputStream in, int PrevDC[], int temp[], int index[]) throws Exception
	{
		...
		for (Cs = 0; Cs < nComp; Cs++)
		{
			...
			for (int i = 0; i < nBlock[Cs]; i++)
			{
				...
				for (int j = 1; j < 64; j++)
				{
					...
					{
						IDCT_Source[IDCT_P[j]] = getn(in, value & 0x0F, temp, index) * qtab[j];
					}
				}
				ScaleIDCT(DU[Cs][i]);
			}
...
class HuffmanTable
{
	...
	int L[][][] = new int[4][2][16], V[][][][] = new int[4][2][16][200]; //tables
