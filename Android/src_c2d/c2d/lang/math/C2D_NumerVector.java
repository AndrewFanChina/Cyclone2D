...
{
	private int capacity = 10;// 容量(初始值是10)
	private int datas[] = new int[capacity];// 内部数组
	private int length = 0;// 存放的元素个数
	/**
	 * 克隆
	 */
	...
	public void shellSort()
	{
		...
		while (h > 0) // 变换跨度，进行多轮插入排序
		{
			...
			for (int iCirc = 0; iCirc < h; iCirc++)// 进行一组跨度为h的h次插入排序
			{
				...
				for (int iSort = iCirc + h; iSort < length; iSort += h)// 进行第iCirc次插入排序
				{
					temp = datas[iSort];
					int iPos = iSort;
