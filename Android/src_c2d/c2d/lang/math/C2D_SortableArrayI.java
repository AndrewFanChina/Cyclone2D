 ...
 *
 */
public class C2D_SortableArrayI
{
	private int m_capacity;// 容量(初始值是10)
	private int m_datas[];// 内部数组
	private int m_length;// 存放的元素个数
	public C2D_SortableArrayI()
	{
		m_capacity = 10;
		m_datas = new int[m_capacity];
	}
	public C2D_SortableArrayI(int capacity)
	{
		m_capacity = capacity;
		m_datas = new int[m_capacity];
	}
	 ...
	 * 克隆
	 */
	public C2D_SortableArrayI cloneSelf()
	{
		C2D_SortableArrayI newInstance = new C2D_SortableArrayI(m_capacity);
	...
	{ // provides workspace
		int[] workSpace = new int[m_length];
	...
	private void recQuickSort1(int left, int right)
	{
		...
		// size is 2 or larger
		{
			int pivot = m_datas[right]; // rightmost item
											// partition range
	...
	{
		int temp = m_datas[dex1]; // A into temp
	...
	private void insertionSort(int left, int right)
	{
		...
		{
			int temp = m_datas[out]; // remove marked item
