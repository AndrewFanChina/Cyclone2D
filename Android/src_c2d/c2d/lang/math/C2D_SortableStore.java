...
/**
 * 转移存储类，基于一个C2D_SortableArray容器，内部含有另外一个C2D_SortableArray，
 * 用于存储从本地容器中“转存”出来的对象，“转存”操作不会删除对象的引用，而是将它们寄存在另外的那个
 * C2D_SortableArray暂存容器中。因此，在今后需要的时候，可以从暂存容器恢复到本地容器。这个
 * 类作用是尽力避免内存的重复卸载和创建。本身的removeXXX操作仍然与C2D_SortableArray
 * 本身相同，都是针对本地容器操作的，如果需要“转存”，这需要使用storeXXX操作。
 * 
 * @author AndrewFan
 * 
 */
public class C2D_SortableStore extends C2D_SortableArray
{
	private C2D_SortableArray storageVector = new C2D_SortableArray();
	public C2D_SortableStore()
	{
		super();
	}
	public C2D_SortableStore(int capacity)
	{
		super(capacity);
	}
	/**
	...
	{
		C2D_SortableStore newInstance = new C2D_SortableStore(m_capacity);
	 ...
	 * @return 原来被暂存的单元
	 */
	public C2D_Order recoverElement()
	{
		C2D_Order storaged = null;
	 ...
	 * @return 被暂存的单元
	 */
	public C2D_Order getStoredElement()
	{
		C2D_Order storaged = null;
		if (storageVector.size() > 0)
		{
